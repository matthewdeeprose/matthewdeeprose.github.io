 ...
 *
 * @description
 * A dependency-free leaf module exposing the single provenance-transition rule
 * shared by the four alt-text write-sites (`_applyValuesToRegistry`,
 * `parseAltText`, `parseCaptions`, `parseAppendix`). Each of those sites used to
 * hard-code a `"user"` provenance stamp; they now route the current stamp
 * through `nextSource(currentSource)` so an AI-authored value that a human then
 * edits is recorded as `"ai-reviewed"` rather than being flattened to `"user"`.
 *
