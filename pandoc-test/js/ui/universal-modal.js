      ...
      buttons.forEach((buttonConfig) => {
        ...
        button.addEventListener("click", async () => {
          ...
          if (typeof buttonConfig.action === "function") {
            try {
              logDebug(
                `Executing button action function for: ${buttonConfig.text}`
              );
              result = await buttonConfig.action();

              // If function returns false, don't close modal
              if (result === false) {
                shouldCloseModal = false;
                logDebug(`Button action returned false, keeping modal open`);
                return;
              }
            } catch (error) {
              logError(`Button action function failed:`, error);
              result = false;
              shouldCloseModal = true;
            }
          }
          // Handle string-based actions (existing logic)
