...
const MusicApp = (function () {
  ...
  // Read a File as text. Returns a Promise that ALWAYS resolves: with the
  // loaded payload on success, or null on read failure. It never rejects, so
  ...
  // .musicxml/.xml/.mxl extension for .pdf, falling back to "score.pdf".
  function pdfFilename(filename) {
    if (typeof filename !== "string" || !filename.length) return "score.pdf";
    return filename.replace(/\.(musicxml|xml|mxl)$/i, "") + ".pdf";
  }

  // The PDF button click: generate a baseline-tagged PDF of the CURRENT render
  // context (so it follows a transpose) and download it. Private, async, and
  // never throws out: every failure path logs, sets pdfStatus and notifies.
  async function handlePdf() {
    // Guard: nothing loaded yet. Report and bail without throwing.
    if (!state.currentModel || !state.currentXml) {
      const msg = "Load a score before downloading a PDF.";
      if (els.pdfStatus) els.pdfStatus.textContent = msg;
      notify.warning(msg);
      return;
    }

    if (els.pdfStatus) els.pdfStatus.textContent = "Generating the tagged PDF. This can take a few seconds.";
    if (els.pdfButton) els.pdfButton.disabled = true; // block a second concurrent compile

    try {
      const bytes = await pdf.generate(state.currentModel, state.currentXml);
      // A null or empty result is a failure, not a download.
      if (!bytes || !bytes.length) {
        logError("PDF generation returned no bytes");
        if (els.pdfStatus) els.pdfStatus.textContent = "The tagged PDF could not be generated.";
        notify.error("The tagged PDF could not be generated.");
        return;
      }
      ...
      const name = pdfFilename(state.filename);
      ...
      if (els.pdfStatus) els.pdfStatus.textContent = "Tagged PDF downloaded.";
      notify.success("Tagged PDF downloaded.");
  ...
  function init() {
    ...
    // The PDF button is static HTML, so it is wired once here (addEventListener,
    // not onclick, because handlePdf is private and not on the public surface).
    if (els.pdfButton) els.pdfButton.addEventListener("click", handlePdf);
