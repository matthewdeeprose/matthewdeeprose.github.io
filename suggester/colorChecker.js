...
export class ColorChecker {
  ...
  getDomElements() {
    try {
      const elements = {
        ...
        backgroundColor: this.getRequiredElement(
          "backgroundColor",
          "Background color display element"
        ),
        backgroundName: this.getRequiredElement(
          "backgroundName",
          "Background color name element"
        ),
        ...
        tColorName: this.getRequiredElement(
          "tColorName",
          "Text color name element"
        ),
        tColorColor: this.getRequiredElement("tColorColor", "Text color box"),
        tcontrast: this.getRequiredElement(
          "tcontrast",
          "Text contrast display"
        ),
        tcontrastWCAG: this.getRequiredElement(
          "tcontrastWCAG",
        ...
        g1colourSpan2: this.getRequiredElement(
          "g1colourSpan2",
          "Graphic 1 color box"
        ),
        g1contrast: this.getRequiredElement("g1contrast", "Graphic 1 contrast"),
        g1contrastWCAG: this.getRequiredElement(
          "g1contrastWCAG",
        ...
        g2colourSpan2: this.getRequiredElement(
          "g2colourSpan2",
          "Graphic 2 color box"
        ),
        g2contrast: this.getRequiredElement("g2contrast", "Graphic 2 contrast"),
        g2contrastWCAG: this.getRequiredElement(
          "g2contrastWCAG",
        ...
        g3colourSpan2: this.getRequiredElement(
          "g3colourSpan2",
          "Graphic 3 color box"
        ),
        g3contrast: this.getRequiredElement("g3contrast", "Graphic 3 contrast"),
        g3contrastWCAG: this.getRequiredElement(
          "g3contrastWCAG",
        ...
        // Display areas
        infoGraphicBox: this.getRequiredElement(
          "infoGraphicBox",
          "Info graphic box"
        ),
  ...
  getRequiredElement(id, description) {
    ...
    if (!element) {
      throw new Error(
        `Required ${description} (ID: ${id}) not found in the DOM`
      );
    }
    return element;
  }
  ...
  async init() {
    ...
    try {
      ...
      // Set up file upload handlers with UIManager reference
      ...
      // Initialize text input handler AFTER UIManager is created
      const textHandler = new ColorTextHandler({
        colorStorage: this.storage,
        uiManager: this.uiManager, // Now this should be properly initialized
      });
  ...
  randomAll() {
    ...
    if (!colorSet) {
      throw new Error(
        "Invalid background color or no valid combinations available"
      );
    }
    ...
    if (this.storage.isColorHeld("text")) {
      ...
      if (!colorSet.textColors.some((c) => c.colourHex === result.text)) {
        ...
        result.text =
          colorSet.textColors[
            Math.floor(Math.random() * colorSet.textColors.length)
          ].colourHex;
      }
    } else {
      result.text =
    ...
    graphicTypes.forEach((type, index) => {
      if (this.storage.isColorHeld(type)) {
        ...
        if (colorSet.graphicColors.some((c) => c.colourHex === heldColor)) {
          ...
          console.warn(
            `Held ${type} color no longer valid with current background`
          );
        }
      }
    });
    ...
    graphicTypes.forEach((type, index) => {
      if (!result.graphics[index]) {
        // If not already set by held color
        if (availableGraphicColors.length === 0) {
          ...
          availableGraphicColors.push(
            ...colorSet.graphicColors.map((c) => c.colourHex)
          );
        }
        const randomIndex = Math.floor(
          Math.random() * availableGraphicColors.length
        );
  ...
  updateColorManagement() {
    ...
    if (!this.initialized) {
      console.warn(
        "Cannot update color management - system not fully initialized"
      );
      return;
    }
    ...
    if (!this.storage || !this.storage.colors || !this.storage.activeColors) {
      console.warn(
        "Cannot update color management - storage not properly initialized"
      );
      return;
    }
